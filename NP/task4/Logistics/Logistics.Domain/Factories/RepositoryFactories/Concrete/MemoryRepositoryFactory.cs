...
public class MemoryRepositoryFactory<T> : IRepositoryFactory<T> where T : EntityBase, IConvert<T>, new()
{
    public IRepository<T> CreateRepository() => new MemoryRepository<T>();
}